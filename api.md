...
至少需要提供以下任意一个参数。如果提供多个不同平台的 ID 参数，或为同一平台提供多个 ID，将使用交集 (AND) 进行匹配。返回匹配结果中最新的一条。

**注意**: `filename` 为独占参数，传入时将忽略其他所有 ID 参数，直接按文件名精确匹配。

如果使用其他平台 ID 获取歌词，将会返回指定 ID 最新的歌词，如果你需要获取旧歌词，请使用 `filename` 精确匹配。
